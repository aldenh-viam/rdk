...
func New(ctx context.Context, deps resource.Dependencies, logger logging.Logger) (Service, error) {
	...
	if err := fs.TBD_InternalReconfigure(ctx, deps, resource.Config{ConvertedAttributes: &Config{}}); err != nil {
		return nil, err
	}
	return fs, nil
}
...
func (svc *frameSystemService) TBD_InternalReconfigure(ctx context.Context, deps resource.Dependencies, conf resource.Config) error {
	svc.partsMu.Lock()
